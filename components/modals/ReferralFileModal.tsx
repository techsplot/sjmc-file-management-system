...
import { ReferralFile, NewReferralFile, FileStatus } from '../../types';
...
const ReferralFileModal: React.FC<ReferralFileModalProps> = ({ isOpen, onClose, onSave, fileToEdit }) => {
  ...
  useEffect(() => {
    if (fileToEdit) {
      setFormData({
        ...
        patientCount: String(fileToEdit.patientCount),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const parsedPatientCount = parseInt(formData.patientCount, 10);

    if (!formData.referralName.trim() || Number.isNaN(parsedPatientCount) || parsedPatientCount <= 0 || !formData.registrationDate || !formData.expiryDate) {
      return;
    }
